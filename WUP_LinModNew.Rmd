...
```
The numerical results conclude with a value of 0.686. The numerical results were given to me from the table above. 
...
I can tell that my scatter plots are accurate because they all relatively have the shape of a straight line. Of course, not perfectly, but this is as accurate as I can get with the model fit. The model fits include Residuals vs. Fitted, Normal Q-Q, Scale Location, and Residuals vs. Leverage. 
...
```{r}
predictSIM = predict.lm(Hi, newdata = data.frame (GRIP = 94, ARM = 88))
print(predictSIM)
```
